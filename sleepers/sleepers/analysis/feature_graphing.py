...
def plot_ft_rotations(
    ...
    Args:
        ...
        plot_non_activating: Whether to plot features that do not activate 
                            (False requires activating features to have defined 'Activations' in the hover_text).
    """
    ...
    if not plot_non_activating:
        hover_text = {k: v for k, v in hover_text.items() if "Activations" in v or 'S' in v}
        del_idx = []  
        # remove non-activating features from hover_text
        for k, v in hover_text.items():
            if 'Activations' in v:
                if len(v['Activations']) == 0:
                    del_idx.append(k)
            elif 'S' in v:
                act_len = len(v['S']) + len(v['M']) + len(v['D']) + len(v['MF']) + len(v['DF'])
                if act_len == 0:
                    del_idx.append(k)
        for idx in del_idx:
            del hover_text[idx]
