...
def load_and_process_data(dataset_name: str, use_sleeper_data: bool, tokenizer, max_steps: int):
    ...
    # Shuffle and select a subset of the data to avoid processing the entire dataset
    data['train'] = data['train'].shuffle(seed=42).select(range(min(max_steps, len(data['train']))))
